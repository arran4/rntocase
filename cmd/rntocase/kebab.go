...
type UsageDataKebab struct {
	*Kebab
	Recursive bool
}

func (c *Kebab) Usage() {
	err := executeUsage(os.Stderr, "kebab_usage.txt", UsageDataKebab{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Kebab) UsageRecursive() {
	err := executeUsage(os.Stderr, "kebab_usage.txt", UsageDataKebab{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Kebab) Execute(args []string) (err error) {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("kebab failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewKebab() *Kebab {
	set := flag.NewFlagSet("kebab", flag.ContinueOnError)
	v := &Kebab{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Kebab) error {
		...
		err := cli.RunKebab(c.outputJSON, c.dryRun, c.interactive, c.recursive, c.include, c.exclude, c.files...)
		if err != nil {
