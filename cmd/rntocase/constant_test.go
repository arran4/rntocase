...
func TestConstant_Execute(t *testing.T) {

	parent := &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewConstant()

	called := false
	cmd.CommandAction = func(c *Constant) error {
		called = true
		return nil
	}
...
func TestConstant_ExecuteHelpAndUnknownFlags(t *testing.T) {
