...
func TestDelimited_Execute(t *testing.T) {

	parent := &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewDelimited()

	called := false
	cmd.CommandAction = func(c *Delimited) error {
		called = true
		return nil
	}
...
func TestDelimited_ExecuteHelpAndUnknownFlags(t *testing.T) {
