...
type UsageDataDelimited struct {
	*Delimited
	Recursive bool
}

func (c *Delimited) Usage() {
	err := executeUsage(os.Stderr, "delimited_usage.txt", UsageDataDelimited{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Delimited) UsageRecursive() {
	err := executeUsage(os.Stderr, "delimited_usage.txt", UsageDataDelimited{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Delimited) Execute(args []string) (err error) {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("delimited failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewDelimited() *Delimited {
	set := flag.NewFlagSet("delimited", flag.ContinueOnError)
	v := &Delimited{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Delimited) error {
		...
		err := cli.RunDelimited(c.delimiter, c.ignore, c.outputJSON, c.dryRun, c.interactive, c.recursive, c.include, c.exclude, c.files...)
		if err != nil {
