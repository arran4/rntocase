...
func TestAcronym_Execute(t *testing.T) {

	parent := &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewAcronym()

	called := false
	cmd.CommandAction = func(c *Acronym) error {
		called = true
		return nil
	}
...
func TestAcronym_ExecuteHelpAndUnknownFlags(t *testing.T) {
