...
type Reverse struct {
	*RootCmd
	...
	CommandAction func(c *Reverse) error
}

type UsageDataReverse struct {
	*Reverse
	Recursive bool
}

func (c *Reverse) Usage() {
	err := executeUsage(os.Stderr, "reverse_usage.txt", UsageDataReverse{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Reverse) UsageRecursive() {
	err := executeUsage(os.Stderr, "reverse_usage.txt", UsageDataReverse{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Reverse) Execute(args []string) (err error) {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("reverse failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewReverse() *Reverse {
	...
	set.BoolVar(&v.wordMode, "word-mode", false, "Reverse words instead of characters")
	...
	v.CommandAction = func(c *Reverse) error {

		err := cli.RunReverse(c.wordMode, c.outputJSON, c.dryRun, c.interactive, c.recursive, c.include, c.exclude, c.files...)
		if err != nil {
