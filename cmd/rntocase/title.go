...
type UsageDataTitle struct {
	*Title
	Recursive bool
}

func (c *Title) Usage() {
	err := executeUsage(os.Stderr, "title_usage.txt", UsageDataTitle{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Title) UsageRecursive() {
	err := executeUsage(os.Stderr, "title_usage.txt", UsageDataTitle{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}
...
func (c *Title) Execute(args []string) error {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("title failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewTitle() *Title {
	set := flag.NewFlagSet("title", flag.ContinueOnError)
	v := &Title{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Title) error {
		...
		err := cli.RunTitle(c.dryRun, c.interactive, c.files...)
		if err != nil {
