...
func TestUpperleading_Execute(t *testing.T) {

	parent := &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewUpperleading()

	called := false
	cmd.CommandAction = func(c *Upperleading) error {
		called = true
		return nil
	}
...
func TestUpperleading_ExecuteHelpAndUnknownFlags(t *testing.T) {
