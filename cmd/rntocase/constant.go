...
type Constant struct {
	*RootCmd
	...
	CommandAction func(c *Constant) error
}

type UsageDataConstant struct {
	*Constant
	Recursive bool
}

func (c *Constant) Usage() {
	err := executeUsage(os.Stderr, "constant_usage.txt", UsageDataConstant{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Constant) UsageRecursive() {
	err := executeUsage(os.Stderr, "constant_usage.txt", UsageDataConstant{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Constant) Execute(args []string) (err error) {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("constant failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewConstant() *Constant {
	set := flag.NewFlagSet("constant", flag.ContinueOnError)
	v := &Constant{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Constant) error {

		err := cli.RunConstant(c.outputJSON, c.dryRun, c.interactive, c.recursive, c.include, c.exclude, c.files...)
		if err != nil {
