...
var _ Cmd = (*Upperleading)(nil)

type Upperleading struct {
	*RootCmd
	Flags         *flag.FlagSet
	outputJSON    bool
	dryRun        bool
	...
	CommandAction func(c *Upperleading) error
}

type UsageDataUpperleading struct {
	*Upperleading
	Recursive bool
}

func (c *Upperleading) Usage() {
	err := executeUsage(os.Stderr, "upperleading_usage.txt", UsageDataUpperleading{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Upperleading) UsageRecursive() {
	err := executeUsage(os.Stderr, "upperleading_usage.txt", UsageDataUpperleading{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Upperleading) Execute(args []string) (err error) {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("upperleading failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewUpperleading() *Upperleading {
	set := flag.NewFlagSet("upperleading", flag.ContinueOnError)
	v := &Upperleading{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Upperleading) error {

		err := cli.RunUpperLeading(c.outputJSON, c.dryRun, c.interactive, c.recursive, c.include, c.exclude, c.files...)
		if err != nil {
