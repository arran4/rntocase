...
func TestLowerleading_Execute(t *testing.T) {

	parent := &RootCmd{
		FlagSet:  flag.NewFlagSet("root", flag.ContinueOnError),
		Commands: make(map[string]func() Cmd),
	}
	cmd := parent.NewLowerleading()

	called := false
	cmd.CommandAction = func(c *Lowerleading) error {
		called = true
		return nil
	}
...
func TestLowerleading_ExecuteHelpAndUnknownFlags(t *testing.T) {
