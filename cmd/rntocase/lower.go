...
type Lower struct {
	*RootCmd
	...
	CommandAction func(c *Lower) error
}

type UsageDataLower struct {
	*Lower
	Recursive bool
}

func (c *Lower) Usage() {
	err := executeUsage(os.Stderr, "lower_usage.txt", UsageDataLower{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Lower) UsageRecursive() {
	err := executeUsage(os.Stderr, "lower_usage.txt", UsageDataLower{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Lower) Execute(args []string) (err error) {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("lower failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewLower() *Lower {
	set := flag.NewFlagSet("lower", flag.ContinueOnError)
	v := &Lower{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Lower) error {

		err := cli.RunLower(c.outputJSON, c.dryRun, c.interactive, c.recursive, c.include, c.exclude, c.files...)
		if err != nil {
