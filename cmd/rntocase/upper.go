...
type UsageDataUpper struct {
	*Upper
	Recursive bool
}

func (c *Upper) Usage() {
	err := executeUsage(os.Stderr, "upper_usage.txt", UsageDataUpper{c, false})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Upper) UsageRecursive() {
	err := executeUsage(os.Stderr, "upper_usage.txt", UsageDataUpper{c, true})
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error generating usage: %s\n", err)
	}
}

func (c *Upper) Execute(args []string) error {
	...
	if c.CommandAction != nil {
		if err := c.CommandAction(c); err != nil {
			return fmt.Errorf("upper failed: %w", err)
		}
	} else {
		c.Usage()
	}
...
func (c *RootCmd) NewUpper() *Upper {
	set := flag.NewFlagSet("upper", flag.ContinueOnError)
	v := &Upper{
		RootCmd:     c,
		Flags:       set,
	...
	v.CommandAction = func(c *Upper) error {
		...
		err := cli.RunUpper(c.dryRun, c.interactive, c.files...)
		if err != nil {
